...
- **导出**：特征 CSV（UTF-8 BOM，Excel 直接开）/HDF5、分段 HDF5/FIF、**预处理后连续数据 EDF/FIF**（单文件面板导出+批处理逐文件，含 epoching 的管线自动跳过），附 JSON 管线溯源 sidecar（步骤参数+文件清单+库版本）
...
5. 特征结果 tab → 导出 CSV / HDF5（sidecar 自动随行）；管线面板「导出连续数据…」把处理后的连续数据落盘 EDF/FIF（批处理导出组亦可勾选逐文件导出）

## 验证

- 单测：`QT_QPA_PLATFORM=offscreen pytest`（287 项，含真实羊 BDF（.edf 误标）与真实 GDF 数据）
