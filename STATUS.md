...
> 本文件回答"现在做到哪了"。每里程碑完成及重要提交后更新。最后更新：2026-09-01（M10 双平台打包 **✅ 完成**：本机 macOS 包 + CI Windows 包均真人验收过，全链路收官）

## 当前里程碑

- **M10 双平台打包（PyInstaller）：✅ 完成（2026-09-01）**——按
  ...
  用户实测验收：macOS 本机"目前 app 使用没有问题" + **Windows 真机双击可运行、未发现
  问题**（2026-09-01，CI artifact win64）——**M10 全项收官**。
...
| M10 双平台打包 | ✅ 完成 | 2026-09-01 | PyInstaller 6.22.2 单份 spec 跨平台 + entry.py shim + app `--smoke` 自检；macOS .app 293MB/zip 119MB/构建 42s，offscreen 冒烟+真窗口存活+PYZ 依赖核实全过、pytest 287 零回归、用户实测验收过；CI 修复（坑 #60）后重跑双绿（win64 136MB/macOS 94MB），**Windows 真机真人验收通过**；瘦身取消；工作区隐私清理（01/02号脑电·羊数据 59 条清出索引） |
...
- 2026-09-01（v0.1.1 发版）：含小屏初始布局修复（坑 #61）——版本 0.1.0→0.1.1
  （pyproject + `__init__.__version__`），tag `v0.1.1` 触发 CI 出双平台包；目的：
  用户 Windows 笔记本复验小屏布局。

- 2026-09-01（M10 后续修复，Windows 真机反馈）：**小屏初始布局截断**——初始窗口硬编
  1440×900 超出笔记本屏幕（1366×768 / 1080p@125% 有效逻辑高 864），右侧处理 Dock 被屏幕
  边缘裁掉（全屏→还原触发重排所以"恢复"）。修复：初始尺寸按主屏可用区域收口+居中 +
  `showEvent` 首显示 `resizeDocks` 按面板当时 hint 定 Dock 宽（坑 #61）。回归测试
  test_ui_window_sizing.py 3 项（小屏收口/大屏保持/125% 高度收口）；pytest **290 绿**（+3）。

- 2026-09-01（M10 收官）：CI 修复（坑 #60）后重跑**双绿**（run 33474440346；artifacts
  win64 136MB / macOS 94MB）；**Windows 真机真人双击验收通过**（用户实测 CI artifact
  win64"可运行使用，未发现问题"）——M10 全项完成翻 ✅。同日工作区隐私清理：01/02号脑电·
  羊数据 59 条清出 `~/.dataloadv` 索引（只清索引不删文件），分发 zip 零命中，默认工作区
  终态 3 来源 1558 条全公共数据。
