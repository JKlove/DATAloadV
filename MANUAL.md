...
>
> **版本基线**：v1（里程碑 M0–M5 全部完成，2026-08-18，commit `f8f6ad7`；
> 验证口径：pytest 137 绿 + e2e_m1–m5 共 78 项 + GUI 冒烟全过）。
...
DataloadV 是面向**介入式 BCI 研究**的电生理数据桌面工作台（macOS 优先，PySide6 深色界面、
全中文），覆盖一条完整工作流：
...
| 波形浏览 | 多通道滚动浏览（峰值包络防混叠）、事件竖线叠加与跳转、事件概览条点击定位、通道显隐、纵向增益、右键坏道标记 | 工作区树/元数据表双击 |
...
启动即深色主题、PingFang SC 中文字体、1440×900 主窗口；未捕获异常自动写入日志文件（§4.1）。

### 2.3 安装验证

| 命令 | 预期 |
|---|---|
| `pytest` | 137 passed（含真实数据项） |
| `python scripts/smoke_gui.py` | 末行 SMOKE OK（真窗口自检后自动退出） |
| `python scripts/e2e_m1.py` … `e2e_m5.py` | 各打印 ALL OK（真实数据端到端，幂等可反复跑） |
...
**没有定义键盘快捷键**——全部操作走菜单、按钮、鼠标（缩放平移见 §3.5）。

### 3.2 五分钟走通（典型流程）

1. **导入**：文件 → 导入文件夹… → 选数据目录（递归扫描，状态栏有进度；失败文件弹错误表）。
2. **浏览**：在元数据表或工作区树**双击**任意一行 → 开浏览 tab，滚轮缩放查看波形与事件线。
...
- **对比 PSD**：独立窗口，双对数坐标，白线=原始、橙线=处理后（通道平均 Welch，取前 120s）。
...
- 无键盘快捷键、无通道拖拽排序、无导出对话框（导出按钮内嵌在结果 tab）——均记 backlog
  见 TODO.md。
